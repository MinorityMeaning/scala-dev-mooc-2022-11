...
object task_collections {
  ...
  def capitalizeIgnoringASCII(text: List[String]): List[String] = {
    text.zipWithIndex.collect {
      case (string, index) if index == 0        => string
      case (string, _) if isASCIIString(string) => string.toUpperCase
      case (string, _)                          => string.toLowerCase
    }
  }
  ...
  def numbersToNumericString(text: String): String = {
    val translatorMap = Map('0' -> "zero",
                            ...
                            '9' -> "nine")
    text.collect{
      case char if translatorMap.contains(char) => translatorMap(char)
